...
// Move pattern array outside the component to prevent re-creation on each render
const PATTERN_ITEMS = [
  '010101',
  '{data}',
  'SELECT',
...
export function CodePattern({
  ...
  useEffect(() => {
    const newStyles = PATTERN_ITEMS.map(() => ({
      opacity: Math.random() * 0.5 + 0.2,
      scale: Math.random() * 0.4 + 0.8
    }));
    setItemStyles(newStyles);
    setIsClient(true);
    // Remove pattern from dependency array since it's now a constant
  }, []);
